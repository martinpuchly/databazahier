    ...
    <AppLayout>
        ...
        <h1>Povolenia skupiny: {{ group.name }}</h1>
            <form @submit.prevent="form.patch(route('admin.permissions.group', group.id))" >
                <div class="row">
                    <div v-for="(permission_group, name) in permissions" v-bind:key="permission_group" class="col">
                        <h2>{{ name }}</h2>
                        <div class="form-check" v-for="permission in permission_group" v-bind:key="permission">
